...
const followersProfileCardButtonFollowLoading = ref(false);
const followersProfileCardButtonUnfollowLoading = ref(false);
...
const Follow = () => {
    followersProfileCardButtonFollowLoading.value = true;
    setTimeout(() => {
        ...
        .then(response_updatedCurrentUser => {
            ...
            followersProfileCardButtonFollowLoading.value = false;
...
const Unfollow = () => {
    followersProfileCardButtonUnfollowLoading.value = true;
    setTimeout(() => {
        axios.delete(`/users/${props.user.userId._id}/unfollow`)
        .then(response_updatedCurrentUser => {
            isCurrentUserFollowOtherUser.value = false,
            store.commit("setUser", {...response_updatedCurrentUser.data, tokens});
            followersProfileCardButtonUnfollowLoading.value = false;
        })
        .catch(() => {
            console.error, 
            isCurrentUserFollowOtherUser.value = true, 
...
onMounted(() => {
    CURRENT_USER.following?.forEach(user => {
        if (user?.userId._id === props.user?.userId._id) {
            isCurrentUserFollowOtherUser.value = true
        }
    });
})
    ...
    <section class="profile-card">
        <div class="action-box">
            <button @click="Follow" v-if="!isCurrentUserFollowOtherUser" class="button-follow">
                ...
                v-if="followersProfileCardButtonFollowLoading" 
                color="#352D2B" 
            ...
            <button @click="Unfollow" v-else class="button-unfollow">
                <AppLoading 
                class="loading-button-follow" 
                v-if="followersProfileCardButtonUnfollowLoading" 
