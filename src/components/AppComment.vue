...
const date = moment(props.comment.created_at + '03:00', 'YYYY-MM-DD HH:mm:ssZ').fromNow();
const buttonDelete = currentUser._id === props.comment.userId._id ? true : false;
const usernameRoute = currentUser._id === props.comment.userId._id ? '/profile' : `/explore/${props.comment.userId.username}`;
...
<template>
    ...
    <article class="post">
        ...
        <section class="post-content">
            <!-- comment replying box -->
            <div class="comment-replying-box">
                <p class="comment-replying">replying to <router-link :to="parentPostUsernameRoute">@{{post.userId.username}}</router-link></p>
            </div>
            ...
            <div class="post-content-inside">
                ...
                <!-- post content text -->
                <p class="post-content-text">{{comment.comment}}</p>
                
                <!-- post content username -->
                <router-link  :to="usernameRoute" class="post-content-username">
                @{{comment.userId.username}}</router-link>
            </div>
        ...
        <section class="post-action">
            ...
            <!-- <router-link :to="`/explore/posts/${post._id}`">
        ...
        <section v-if="isDeleteModal" class="delete-comment-modal">
            <h2 class="delete-comment-modal-question">Are you sure about deleting your comment?</h2>
...
.delete-comment-modal-button-yes {
    padding: 5px 10px;
    font-weight: 600;
    color: var(--light);
...
.delete-comment-modal-button-yes:focus {
    outline: var(--red) solid 2px;
    outline-offset: 2px;
}
.delete-comment-modal-button-yes:active {
...
.delete-comment-modal-button-no {
    padding: 5px 10px;
    font-weight: 600;
    color: var(--dark);
    background-color: transparent;
...
.delete-comment-modal-button-no:focus {
    outline: var(--dark) solid 2px;
    outline-offset: 2px;
}
.delete-comment-modal-button-no:active {
    outline: var(--dark) solid 2px;
}
</style>